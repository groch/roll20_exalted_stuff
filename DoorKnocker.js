    ...
    const version = 1.3,
      ...
      checkInstall = function () {
        try {
          ...
          if (debug) log(`error from checkInstall`);
          sendError(err);
        }
      },
      ...
      updateHelpCharacterProperties = function () {
        ...
        let expectedAbilities = {},
          abilityTemplates = {
            open:       {name: 'Open Doors',            action: '!knock --key|open'},
            close:      {name: 'Close Doors',           action: '!knock --key|close'},
            toggle:     {name: 'Toggle Doors',          action: '!knock --key|toggle'},
            pushOpen:   {name: 'Open Unlocked Doors',   action: '!knock --push|open'},
            pushClosed: {name: 'Close Unlocked Doors',  action: '!knock --push|close'},
            pushToggle: {name: 'Toggle Unlocked Doors', action: '!knock --push|toggle'}
          };
          Object.keys(abilityTemplates).forEach((key, index) => {
            expectedAbilities[key] = false;
            abilityTemplates[key]['description'] = `Door Knocker Ability:${key}`;
            abilityTemplates[key]['characterid'] = state.DoorKnocker.help;
