...
//Script Constants
    ...
    handlePath = function(obj,prev){
        ...
        // I ADDED THIS
        let rgbRegex = /rgb\((\d{1,3}),\s?(\d{1,3}),\s?(\d{1,3})\)/, ret;
        if ((ret = obj.get('stroke').match(rgbRegex))) {
            let finalStroke = `#${Number(ret[1]).toString(16).padStart(2, '0')}${Number(ret[2]).toString(16).padStart(2, '0')}${Number(ret[3]).toString(16).padStart(2, '0')}`;
            obj.set('stroke', finalStroke);
        }
        // I ADDED ABOVE THIS
